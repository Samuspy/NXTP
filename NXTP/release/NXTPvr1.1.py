...
from NXTP_core.msn_engine import build_handshake_payload, mutual_verification_probe 
...
def main():
    target = input("Enter domain name or IP to evaluate: ").strip().lower()
    try:
    ...
    # Greet Packet & Handshake Prep
    greet_packet = build_greet_packet(target, [443], tls_info, whois_info)
    node_id = greet_packet["node_id"]
    intent_tag = greet_packet["intent_tag"]
    score = verdict_data.get("score", risk_score)

    payload = build_handshake_payload(node_id, score)
    msg_verified, msg_data = mutual_verification_probe(domain, payload)

    # Output Sections
    print("=" * 20, "NXTP INITIAL GREET", "=" * 20)
    ...
    print("=" * 22, "MSG VERIFICATION", "=" * 22)
    if msg_verified:
        print("Status          : Mutual security verified")
        print(f"Peer Engine     : {msg_data.get('verdict_engine', 'Unknown')}")
        score -= 1
        intent_tag += "|mutual"
    else:
        print("Status          : Mutual security failed or unresponsive")
        print(f"Error Info      : {msg_data.get('error', 'No response received')}")
        score += 1
    print("=" * 54)
...
from NXTP_core.session_memory import log_node_session


if __name__ == "__main__":
    print("[NXTP Runtime Booting]")
    main()
